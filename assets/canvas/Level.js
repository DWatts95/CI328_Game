...
Level.prototype.preload = function () {
	...
	this.load.image('fireSpell', 'assets/images/fireblast1.png');
	this.load.image('jumpControl', 'assets/text/jumpControl.png');
	this.load.image('bootPickup', 'assets/text/bootPickup.png');
	this.load.image('movement', 'assets/text/movement.png');
	this.load.image('shootControl', 'assets/text/shootControl.png');
	this.load.audio('musicMP3', 'assets/sound/battleMusic.mp3');
	this.load.audio('fireNoise', 'assets/sound/fireNoise.wav');
	this.load.audio('deathNoiseWAV' , 'assets/sound/deathNoise.wav');
...
Level.prototype.create = function () {
	...
	music = this.game.add.audio('musicMP3');
	music.play();
	
	fireNoise = this.game.add.audio('fireNoise');
	deathNoise = this.game.add.audio('deathNoiseWAV');
	...
	var jumpControl = this.add.image(0, -1000,'jumpControl');
	var bootPickup = this.add.image(0, -1000,'bootPickup');
	var movement = this.add.image(0, -1000,'movement');
	var shootControl = this.add.image(0, -1000,'shootControl');
	...
	this.fJumpControl = jumpControl;
	this.fBootPickup = bootPickup;
	this.fMovement = movement;
	this.fShootControl = shootControl;
	...
	this.fMovement.x = 6;
	this.fMovement.y = 591;
	
	this.fJumpControl.x = 1078;
	this.fJumpControl.y = 775;
	
	this.fShootControl.x = 1500;
	this.fShootControl.y = 650; 
...
Level.prototype.update = function () {
    ...
    if (this.fNobleRanger.x - this.fPyroOrc.x <400+rangeIncrease && this.fNobleRanger.x - this.fPyroOrc.x > -400-rangeIncrease && this.fNobleRanger.y - this.fPyroOrc.y > (-100+(2*enemyAccuracy))  && this.fNobleRanger.y - this.fPyroOrc.y < (50-enemyAccuracy)){
...
Level.prototype.collideFunction = function(){
	...
	this.fPyroOrc.revive();
		...
		this.fPyroOrc.x = 0;
		this.game.add.text(this.fNobleRanger.x-100, this.fNobleRanger.y -60, 'well done you have completed the game.', {"font":"bold 20px Arial"});
	}
	console.log(count);
	
};
Level.prototype.chestPickup = function(){
	jumpHeight = -400;
	this.fPickupChest.kill();
	this.fBootPickup.x = 3250;
	this.fBootPickup.y = 750;
	this.fPickupChest.revive();
};

Level.prototype.killPlayer = function(){
	...
	this.fNobleRanger.kill();
		...
		this.fBootPickup.x = 0;
		this.fBootPickup.y = -1000;
		jumpHeight = -250;
		enemyAccuracy = 0;
		speedIncrease = 0;
		rangeIncrease = 0;

		this.fNobleRanger.x = 103;
		this.fNobleRanger.y = 571;
		this.fPyroOrc.x = 1850;
		this.fPyroOrc.y = 650;
		this.fPyroOrc.kill();
		this.fMovement.kill();
		this.fJumpControl.kill();
		this.fShootControl.kill();
		count = 0;
		score = 0;
		this.fPyroOrc.revive();
		this.fNobleRanger.revive();
